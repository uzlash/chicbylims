    ...
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-4">
        {item.imgs?.previews?.[0] ? (
          <Image src={item.imgs.previews[0]} alt={item.title || "Product Image"} width={250} height={250} className="object-contain" />
        ) : (
          <div className="flex items-center justify-center w-full h-[270px] bg-gray-2 text-gray-500">
            No Image
          </div>
        )}
      ...
      {item.productOfMonth && (
        <span className="inline-flex items-center px-2 py-0.5 mb-1 rounded-full bg-orange/10 text-orange text-[11px] font-semibold uppercase tracking-wide">
      ...
      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">{symbol}{formatPrice(displayPrice, currency)}</span>
        <span className="text-dark-4 line-through">{symbol}{formatPrice(displayCompare, currency)}</span>
      </span>
    </div>
  );
};
