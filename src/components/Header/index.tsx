...
const Header = () => {
  ...
  const { currency, setCurrency, toDisplayAmount, symbol } = useCurrency();

  const brandPrefix = "";
  const accentColor = "text-blue";
  const hoverAccentColor = "hover:text-blue";
  const bgAccentColor = "bg-blue";
  const beforeBgAccentColor = "before:bg-blue";
  ...
  return (
    ...
    >
      <div className="max-w-[1170px] mx-auto px-3 sm:px-7.5 xl:px-0">
        {/* <!-- header top start --> */}
        <div
          className={`flex flex-wrap items-center gap-2 sm:gap-3 xl:gap-5 xl:justify-between ease-out duration-200 ${
            stickyMenu ? "py-2.5 xl:py-4" : "py-3 xl:py-6"
          }`}
        >
          {/* Logo: row 1 on mobile; first column on xl */}
          <Link
            className="order-1 flex-shrink-0 xl:flex-shrink-0"
            href="/"
          >
            <Image
              src="/chibylims_logo.png"
              alt="Chibylims"
              width={160}
              height={40}
          ...
          <div className="order-2 ml-auto flex items-center gap-1.5 sm:gap-2 shrink-0 xl:hidden">
            ...
            {!session ? (
              <Link
                href="/signin"
                className="flex h-10 w-10 items-center justify-center rounded-md text-blue hover:bg-gray-1"
                aria-label="Sign in"
              >
                <svg
                  width="22"
              ...
              <Link
                href="/my-account"
                className="flex h-10 w-10 items-center justify-center rounded-md text-blue hover:bg-gray-1"
                aria-label={session.user?.name ? `Account, ${session.user.name}` : "My account"}
              >
                <svg
                  width="22"
                  height="22"
            ...
            <button
              ...
              className="relative flex h-10 w-10 items-center justify-center rounded-md text-blue hover:bg-gray-1"
            ...
            >
              ...
              <span
                className={`flex items-center justify-center font-medium text-2xs absolute -right-0.5 -top-0.5 min-w-[1.125rem] h-4.5 px-0.5 rounded-full text-white ${bgAccentColor}`}
              >
                {product.length}
              </span>
            </button>
            ...
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-md hover:bg-gray-1"
          ...
          {/* Search: full width row on mobile; beside logo on xl */}
          <div className="order-3 w-full min-w-0 xl:order-2 xl:w-auto xl:max-w-[520px] xl:flex-1">
            <form onSubmit={handleSearch}>
              <div className="flex items-center">
                <div className="relative w-full">
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 inline-block w-px h-5.5 bg-gray-4 max-xl:hidden"></span>
                  <input
                    ...
                    autoComplete="off"
                    className="custom-search w-full rounded-r-[5px] bg-gray-1 max-xl:rounded-[5px] max-xl:border-l border border-gray-3 py-2 pl-3 pr-10 text-sm outline-none ease-in duration-200 xl:!border-l-0 xl:py-2.5 xl:pl-4 xl:text-base"
                  />

                    <button
                      id="search-btn"
                      aria-label="Search"
                      className={`flex items-center justify-center absolute right-3 top-1/2 -translate-y-1/2 ease-in duration-200 ${hoverAccentColor}`}
                    >
                      <svg
                        className="fill-current"
                        width="18"
          ...
          {/* <!-- header top right (desktop) --> */}
            <div className="order-4 hidden w-full flex-none items-center justify-end gap-5 lg:gap-7.5 xl:flex xl:w-auto">
            <div className="hidden xl:flex items-center gap-3.5 flex-shrink-0">
              <div className="inline-flex items-center gap-1 rounded-full border border-gray-3 bg-white/80 backdrop-blur px-1 py-0.5">
                <button
                  type="button"
                  onClick={() => setCurrency("NGN")}
                  className={`px-2.5 py-1 text-custom-sm font-medium rounded transition-colors ${
                    currency === "NGN" ? "bg-blue text-white" : "text-dark-4 hover:bg-gray-1"
                  }`}
                >
                  ₦ NGN
                </button>
                <button
                  ...
                  onClick={() => setCurrency("USD")}
                  className={`px-2.5 py-1 text-custom-sm font-medium rounded transition-colors ${
                    currency === "USD" ? "bg-blue text-white" : "text-dark-4 hover:bg-gray-1"
                  }`}
                >
                  $ USD
                </button>
              ...
              <div className="flex items-center gap-5">
                ...
                >
                  <span className="inline-block relative">
                    ...
                    <span className={`flex items-center justify-center font-medium text-2xs absolute -right-2 -top-2.5 w-4.5 h-4.5 rounded-full text-white ${bgAccentColor}`}>
                      {product.length}
                    </span>
                  </span>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-custom-sm text-dark">
                      {symbol}{formatPrice(displayTotal, currency)}
                    </p>
                  </div>
                </button>
              </div>
            ...
            <div className="hidden xl:block">
              <ul className="flex items-center gap-5.5">
                <li className="py-4">
                  <Link
                    href="/wishlist"
                    className={`flex items-center gap-1.5 font-medium text-custom-sm text-dark ${hoverAccentColor}`}
                  >
                    <svg
                      className="fill-current"
                      width="16"
